...
class AppDelegate: FlutterAppDelegate {
  ...
  override func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    let channel = FlutterMethodChannel(name: calendarChannel, binaryMessenger: controller.engine.binaryMessenger)
  ...
  private func getCalendars(result: @escaping FlutterResult) {
    ...
    let calendarData = calendars.map { calendar -> [String: Any] in
      return [
        ...
        "name": calendar.title,
        "color": calendar.cgColor?.components?.first != nil ? 0 : 0,
