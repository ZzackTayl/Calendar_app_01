...
PROD_SUPABASE_ANON_KEY=your-production-anon-key-here
FLUTTER_ENV=prod
EOF
...
flutter run --dart-define=FLUTTER_ENV=prod \
             --dart-define=PROD_SUPABASE_URL=production_url \
             --dart-define=PROD_SUPABASE_ANON_KEY=production_key
```
...
grep 'PROD_SUPABASE_URL=' .env.production

# ✓ Anon key (not service role key)
grep 'PROD_SUPABASE_ANON_KEY=' .env.production
```
