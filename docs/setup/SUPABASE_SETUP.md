...
Create a temporary note with your development credentials:
```
DEV_SUPABASE_URL=https://your-project-url.supabase.co
DEV_SUPABASE_ANON_KEY=your-long-anon-key-here
```
...
**Before:**
```env
DEV_SUPABASE_URL=your-dev-supabase-url-here
DEV_SUPABASE_ANON_KEY=your-dev-supabase-anon-key-here
```
...
**Building without the `.env` file?**  
When you run on CI or for staging/production, supply the same values with `--dart-define` so nothing sensitive gets bundled with the app:
```bash
flutter run --dart-define=FLUTTER_ENV=staging \
             --dart-define=STAGING_SUPABASE_URL=https://your-staging.supabase.co \
             --dart-define=STAGING_SUPABASE_ANON_KEY=staging-anon-key
```
...
### ✅ SAFE TO COMMIT:
- `DEV_SUPABASE_URL`, `STAGING_SUPABASE_URL`, `PROD_SUPABASE_URL` (documentation examples only)
- `DEV_SUPABASE_ANON_KEY`, `STAGING_SUPABASE_ANON_KEY`, `PROD_SUPABASE_ANON_KEY` (anon/public keys are safe, but keep real values in a private env file)
