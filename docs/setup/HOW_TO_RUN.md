...
- **Local development:** keep a `.env` file in the project root (not committed) with `FLUTTER_ENV=dev`, `DEV_SUPABASE_URL`, `DEV_SUPABASE_ANON_KEY`, etc. The launcher scripts load it automatically.
- **CI / staging / production builds:** pass secrets with `--dart-define` instead of copying `.env` into the app bundle. Example:
  ```bash
  ...
  ```
- Keep sensitive values like Twilio auth tokens on the server or secret manager only—do not ship them in the mobile app.
