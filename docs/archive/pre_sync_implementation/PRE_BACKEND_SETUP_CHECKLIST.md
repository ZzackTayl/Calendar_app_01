...
Should look like:
```dart
static String get supabaseUrl => switch (flutterEnv) {
  'prod' => _value('PROD_SUPABASE_URL'),
  'staging' => _value('STAGING_SUPABASE_URL'),
  _ => _value('DEV_SUPABASE_URL'),
};
static String get supabaseAnonKey => switch (flutterEnv) {
  'prod' => _value('PROD_SUPABASE_ANON_KEY'),
  'staging' => _value('STAGING_SUPABASE_ANON_KEY'),
  _ => _value('DEV_SUPABASE_ANON_KEY'),
};
```
