  ...
  group('RecurrenceRule', () {
    ...
    group('Weekly Recurrence', () {
      test('should generate weekly occurrences on same day', () {
        ...
        expect(occurrences.every((date) => date.weekday == DateTime.monday), isTrue);
      });
      ...
      test('should generate weekly occurrences on specific days', () {
        ...
        expect(occurrences.last, equals(baseDate.add(const Duration(days: 14))));

        // Check that all occurrences are on correct days
        for (final occurrence in occurrences) {
          expect(
              [DateTime.monday, DateTime.wednesday, DateTime.friday].contains(occurrence.weekday),
              isTrue);
        }
      });
    ...
    group('Monthly Recurrence', () {
      ...
      test('should generate monthly occurrences on last day', () {
        ...
        expect(occurrences[1], equals(DateTime(2024, 2, 29))); // Feb 29 (leap year)
    ...
    group('Yearly Recurrence', () {
      test('should generate yearly occurrences correctly', () {
        ...
        expect(occurrences.every((date) => date.month == 6 && date.day == 15), isTrue);
      });
    ...
    group('Exceptions', () {
      test('should skip exception dates', () {
        ...
        expect(occurrences, isNot(contains(baseDate.add(const Duration(days: 2)))));
        expect(occurrences, isNot(contains(baseDate.add(const Duration(days: 4)))));
      });
    });
    ...
    group('JSON Serialization', () {
      test('should serialize and deserialize correctly', () {
        ...
        expect(deserializedRule.occurrenceCount, equals(originalRule.occurrenceCount));
        expect(deserializedRule.exceptions, hasLength(1));
        expect(deserializedRule.createdAt.isAtSameMomentAs(originalRule.createdAt), isTrue);
      });
    });
    ...
    group('Description Generation', () {
      test('should generate correct daily descriptions', () {
        ...
        expect(RecurrenceRulePatterns.daily(interval: 3).description, equals('Every 3 days'));
      });

      test('should generate correct weekly descriptions', () {
        expect(RecurrenceRulePatterns.weekly().description, equals('Weekly'));
        expect(RecurrenceRulePatterns.weekly(interval: 2).description, equals('Every 2 weeks'));
        expect(
            RecurrenceRulePatterns.weekly(daysOfWeek: [WeekDay.monday, WeekDay.friday]).description,
      ...
      test('should generate correct monthly descriptions', () {
        ...
        expect(RecurrenceRulePatterns.monthly(interval: 3).description, equals('Every 3 months'));
        expect(
            RecurrenceRulePatterns.monthly(monthlyPattern: MonthlyPattern.sameWeekday).description,
            equals('Monthly on the same weekday'));
        expect(RecurrenceRulePatterns.monthly(monthlyPattern: MonthlyPattern.lastDay).description,
      ...
      test('should generate correct yearly descriptions', () {
        ...
        expect(RecurrenceRulePatterns.yearly(interval: 2).description, equals('Every 2 years'));
      });
