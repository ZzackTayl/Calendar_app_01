...
import 'package:myorbit_calendar/features/calendar_sharing/presentation/cubit/calendar_sharing_cubit.dart';
...
class _CalendarSharingScreenState extends State<CalendarSharingScreen> {
  ...
  Future<void> _sendShareInvites() async {
    ...
    final sharingCubit = context.read<CalendarSharingCubit>();
    ...
    await sharingCubit.sendShareInvites(
    ...
    final state = sharingCubit.state;
      ...
      unawaited(
        AnalyticsService.logCustomEvent(
          ...
          parameters: <String, Object?>{
            ...
            'can_edit': _canEditEvents,
            'share_availability': _shareAvailability,
            'reason': _sanitizeAnalyticsMessage(state.message),
          },
        ),
      );
      ScaffoldMessenger.of(context)
        ...
        child: SafeArea(
          ...
          child: BlocBuilder<ContactCubit, ContactState>(
            builder: (context, contactState) {
              ...
              final isLoadingContacts = contactState.status.isLoading;
              final contactsError = contactState.status.isFailure
                  ? contactState.message
                  : null;

              return BlocBuilder<CalendarSharingCubit, CalendarSharingState>(
                builder: (context, sharingState) {
                  final isSubmitting =
                      _isSubmitting || sharingState.status.isLoading;
                  ...
                  return Stepper(
                    ...
                    controlsBuilder: (context, details) {
                      ...
                      return Row(
                        children: [
                          FilledButton.icon(
                            onPressed:
                                isSubmitting ? null : details.onStepContinue,
                            icon: isSubmitting && isLastStep
                                ...
                                : Icon(isLastStep
                                    ...
                                    : Icons.arrow_forward),
                            label:
                                Text(isLastStep ? 'Send invites' : 'Continue'),
                          ),
                          const SizedBox(width: 12),
                          TextButton(
                            onPressed:
                                isSubmitting ? null : details.onStepCancel,
                    ...
                    steps: [
                      ...
                      _buildSetPermissionsStep(context),
                      _buildReviewStep(context, palette),
                    ],
                  );
                },
              );
  ...
  Step _buildSetPermissionsStep(BuildContext context) {
    return Step(
  ...
  Step _buildReviewStep(BuildContext context, AppPalette palette) {
    return Step(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildPermissionChip(context,
                      enabled: _canEditEvents, label: 'Can suggest edits'),
                  _buildPermissionChip(context,
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: 'Add a note to include in the invite email',
                    ),
                  ),
                ],
              ),
